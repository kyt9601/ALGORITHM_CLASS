...
void DijkstraShortestPaths(graph *pg, int n, int start);
...
void DijkstraShortestPaths(graph *pg, int n, int start)	// Prim-Jarnik 알고리즘
{
	vertex *s;
	...
	pg->verticies[start-1].distance = 0;
	...
	while (!isEmpty(&queue))
	{
		...
		for (incident *pi=u->incidentHead->next; pi!=NULL; pi=pi->next)
		{
			...
			if (z->inPriorityQueue && u->distance+pi->pe->weight < z->distance)
				replaceKey(&queue, z, u->distance+pi->pe->weight);	// z의 idxPriorityQueue 멤버를 통해 우선순위 큐 내에서의 인덱스를 상수 시간에 확인 가능하다.
		}
	}
	puts("");
	...
	for (int i=0; i<n; i++)
		if (i+1 != start && pg->verticies[i].distance < INFINITE)
